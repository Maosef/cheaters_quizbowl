...
class Questions:
    ...
    def get_sent(self, qid, sent_index):
        ...
        # print(bounds)
        # print(self.questions[qid]['question'])
...
# concat all evidence into one string
def evidences_to_string(evidences):
    return ' '.join([evidence.text for evidence in evidences ])
    ...
    for ln in f:
        ...
        if not ex['skipped']:
            ...
            for action_num,gold in enumerate(tr_example.gold_actions):
                ...
                evidences = evidences_to_string(gold[1])
                action = gold[2]
                action_name = gold[2].split(':')[0]
                ...
                if key not in question_set:
                    num_questions += 1
                    question_set.add(key)
                # if we've seen the question, add to the evidence
                if key in gold_actions and 

                # add new questions with the last query before answering correctly
                if key not in added_actions and \
                    action_name == 'query' and \
                    ex['answer_correct'] and \
                    num_queries-cur_query == 0:
...
with open('qb-gold-rtvr-input.json', 'w') as oh: json.dump(gold_actions, oh, indent=4)

# print('total: {}, correct: {}'.format(total, num_correct))
# print('num-actions: {}'.format(num_actions))
print('number of unique successful question-query pairs: {}'.format(len(gold_actions)))
print('number of unique questions: {}'.format(num_questions))
